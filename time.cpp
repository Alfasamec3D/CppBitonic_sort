...
int main() {
  ...
  //std::ifstream input_file("tests/e2e/io/003.dat");
  const int LOCAL = 256;
  int N;
  std::cin >> N;
  assert((N & (N - 1)) == 0);
  ...
  for (unsigned int k = LOCAL * 2; k <= N; k <<= 1) {
    for (unsigned int j = k >> 1; j > 0; j >>= 1) {
      ...
      queue.enqueueNDRangeKernel(kernelMerge, cl::NullRange, global);
    }
  }
  queue.finish();
  ...
  std::cout << "GPU time measured: " << timeGPU << " ms" << std::endl;
  std::cout << "CPU time measured: " << timeCPU << " ms" << std::endl;
